...
module.exports = {
  ...
  async fn(inputs, exits) {
    try {
      ...
      if (oldCandidate.data && updatedCandidate.data && oldCandidate.data.candidateUpdates != updatedCandidate.data.candidateUpdates) {
        const candidateSupports = await Support.find({
          candidate: id,
        }).populate('user');
        const { race, firstName, lastName } = updatedCandidate || {};
