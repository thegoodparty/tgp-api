      ...
      for (let i = 0; i < campaigns.length; i++) {
        ...
        if (positionId) {
          ...
          if (position?.data?.hasPrimary) {
            csvRows += `\n${campaign.id},${campaign.slug},${campaign.data?.details?.firstName} ${campaign.data?.details?.lastName},${position.id},${campaign?.data?.details?.electionId},${campaign?.data?.details?.raceId},${campaign?.data?.details?.electionDate},${campaign?.data?.details?.state},${campaign?.data?.details?.otherOffice}<br/>`;
          }
        }
      }
